...
begin
  ...
  for i := btab[1].last to t do
    with tab[i] do
      Writeln(psout, i, ' ', Name, link: 5, Ord(obj): 5,
        Ord(typ): 5, ref: 5, Ord(normal): 5, lev: 5, adr: 5);
  Writeln(psout);
  Writeln(psout);
  Writeln(psout);
        ...
        begin { field section }
          ...
          begin
            ...
            if sy = colon then
              ...
              error(5);
            t1 := t;
            typ(fsys + [semicolon, endsy, comma, ident], eltp,
              elrf, elsz);
            while t0 < t1 do
            begin
              t0 := t0 + 1;
    ...
    case ir.f of
      ...
      begin  { write1 }
        ...
        if chrcnt > lineleng then
          ...
          case ir.y of
            1:
              Write(prr, s[t].i: fld[1]);
            2:
              Write(prr, s[t].r: fld[2]);
            3:
              if s[t].b then
                Write('true')
              else
    ...
    case ir.f of
      ...
      begin { write2 }
        ...
        if chrcnt > lineleng then
          ...
          case ir.y of
            1:
              Write(prr, s[t - 1].i: s[t].i);
            2:
              Write(prr, s[t - 1].r: s[t].i);
            3:
              if s[t - 1].b then
                Write('true')
              else
...
begin  { interpret }
  ...
  begin
    ...
    case ps of
      caschk:
        Writeln(prr, 'undefined case');
      divchk:
        Writeln(prr, 'division by 0');
      inxchk:
        Writeln(prr, 'invalid index');
      stkchk:
        Writeln(prr, 'storage overflow');
      linchk:
        Writeln(prr, 'too much output');
      lngchk:
        Writeln(prr, 'line too long');
      redchk:
        Writeln(prr, 'reading past end or file');
    end;
    h1     := b;
